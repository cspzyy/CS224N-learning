...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    func = softmax(np.dot(centerWordVec, outsideVectors.T))
    delta = func.copy()
    delta[outsideWordIdx] -= 1

    loss = -np.log(func)[outsideWordIdx]
...
def negSamplingLossAndGradient(
    ...
    unique = outsideVectors[negSampleWordIndices]
    sigm = sigmoid(np.dot(unique, centerWordVec))
    loss += np.sum(-np.log(sigm))
    gradCenterVec += -np.dot((sigm-1), unique)
    gradOutsideVecs[negSampleWordIndices] += -np.outer((sigm-1), centerWordVec)
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        (single_loss, gradCenter, gradOutside) = word2vecLossAndGradient(currentCenterWordVec, outsideWordIdx, outsideVectors, dataset)
        loss += single_loss
