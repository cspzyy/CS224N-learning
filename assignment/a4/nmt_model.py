...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTM(input_size = embed_size + hidden_size, hidden_size = hidden_size, bias = True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # 按照规定形状使用source model embeddings构建X
        X = self.model_embeddings.source(source_padded)
        # encoder部分，首先使用pack_padded_sequence函数对X进行包装，然后使用encoder进行计算
        X = pack_padded_sequence(X, source_lengths)
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)
            # 交换维度，改变encoder_hiddens的形状
        enc_hiddens = pad_packed_sequence(enc_hiddens)[0].permute(1, 0, 2)
        # 计算decoder初始状态
        hidden_encoder_cat = torch.cat([last_hidden[0], last_hidden[1]], dim=1)
        init_decoder_hidden = self.h_projection(hidden_encoder_cat)
        cell_encoder_cat = torch.cat([last_cell[0], last_cell[1]], dim=1)
        init_decoder_cell = self.c_projection(cell_encoder_cat)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 1. Apply softmax to e_t to yield alpha_t
        alpha_t = F.softmax(e_t, dim=1).unsqueeze(dim=1)
        # 2. Use batched matrix multiplication between alpha_t and enc_hiddens to obtain a_t
        a_t = alpha_t.bmm(enc_hiddens).squeeze(dim=1)
        # 3. Concatenate dec_hidden with a_t to compute U_t
        U_t = torch.cat([dec_hidden, a_t], dim=1)
        # 4. Apply the combined output projection layer to U_t to compute V_t
        V_t = self.combined_output_projection(U_t)
        # 5. Compute O_t by first applying the Tanh function and then the dropout layer
