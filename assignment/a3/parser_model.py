...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # 定义一个线性层，将词向量映射到隐藏层
        self.embed2hidden = nn.Linear(n_features * self.embed_size, self.hidden_size)
        # 初始化这个线性层的weight和bias
        nn.init.xavier_uniform_(self.embed2hidden.weight)
        nn.init.uniform_(self.embed2hidden.bias)
        # 初始化dropout层
        self.dropout = nn.Dropout(p=self.dropout_prob)
        # 同样地，定义一个线性层，将隐藏层映射到输出层
        self.hidden2logits = nn.Linear(self.hidden_size, self.n_classes)
        nn.init.xavier_uniform_(self.hidden2logits.weight)
        nn.init.uniform_(self.hidden2logits.bias)
    ...
    def embedding_lookup(self, w):
        ...
        # 取出三个塑形数据batch_size, n_features, embed_size
        batch_size, n_features = w.size()
        embedding_size = self.embeddings.size(1)
        x = vec.view(batch_size, n_features * embedding_size)
    ...
    def forward(self, w):
        ...
        # 转化为隐藏层
        hidden_layer = self.embed2hidden(embeddings)
        # 激活函数
        hidden_activation = F.relu(hidden_layer)
        # 进行dropout
        drop_out = self.dropout(hidden_activation)
        # 转化为logits
        logits = self.hidden2logits(drop_out)
        ### END YOUR CODE
        return logits
